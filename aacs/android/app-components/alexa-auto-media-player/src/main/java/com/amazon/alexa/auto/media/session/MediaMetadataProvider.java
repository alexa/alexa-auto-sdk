...
public class MediaMetadataProvider {
    ...
    private static Optional<String> getAlbumArtUrl(@NonNull RenderPlayerInfo metadata) {
        ...
        String[] sizePriority = new String[] {// First priority is to find x-large image, and so on.
                TemplateRuntimeConstants.IMAGE_SIZE_XLARGE, TemplateRuntimeConstants.IMAGE_SIZE_LARGE,
    ...
    private static Optional<RatingCompat> getRatingObject(@NonNull RenderPlayerInfo metadata) {
        ...
        if (metadata.getPayload().getControls() != null) {
        ...
        Log.w(TAG, "Playback control data is null, skip to build rating object.");
        return Optional.empty();
    }
