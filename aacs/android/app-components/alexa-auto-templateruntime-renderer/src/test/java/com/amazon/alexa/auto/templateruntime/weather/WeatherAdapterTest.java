...
public class WeatherAdapterTest {
    ...
    public static final PorterDuffColorFilter COLOR_FILTER_WHITE = new PorterDuffColorFilter(COLOR_WHITE, PorterDuff.Mode.SRC_ATOP);
    ...
    public void testOnBindViewHolder_populatesWeatherForecastDataCorrectly() {
        try (MockedStatic<Picasso> staticMock = Mockito.mockStatic(Picasso.class)) {
            ...
            for (int i = 0; i < WEATHER_FORECAST_DAYS_COUNT; i++) {
                ...
                assertEquals(weatherForecastTempHigh.getText(), weatherTemplate.getWeatherForecast().get(i).getHighTemperature());
                TextView weatherForecastTempLow = view.findViewById(R.id.weather_card_forecast_temp_low);
                assertEquals(weatherForecastTempLow.getText(), weatherTemplate.getWeatherForecast().get(i).getLowTemperature());
            }
    ...
    private WeatherTemplate weatherTemplate() {
        return new WeatherTemplate(
                "WeatherTemplate",
                "88ade3ee-dc44-42ab-a894-e613a4d5c9ab",
                new Temperature(
                        "61°",
                        new DisplayAsset(
                                "Down arrow",
