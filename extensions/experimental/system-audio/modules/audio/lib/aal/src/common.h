...
typedef struct {
	aal_handle_t (*create) (const aal_attributes_t *attr);
	...
	ssize_t (*write) (aal_handle_t handle, const char *data, const size_t size);
	void (*notify_end_of_stream) (aal_handle_t handle);
	void (*set_stream_type) (aal_handle_t handle, const aal_stream_type_t type);
