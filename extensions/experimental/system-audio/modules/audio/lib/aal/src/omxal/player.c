...
static aal_handle_t omxal_player_create(const aal_attributes_t *attr)
{
	...
	XADataSource source = {
		.pLocator = &source_locator,
		.pFormat = NULL
	};

	XADataLocator_OutputMix sink_locator = {
		.locatorType = XA_DATALOCATOR_OUTPUTMIX,
		.outputMix = NULL
	};
	XADataSink sink = {
		.pLocator = &sink_locator,
		.pFormat = NULL
	};

	XAInterfaceID interface_ids[] = {
		XA_IID_VOLUME
	};
	XAboolean required_list[] = {
		XA_BOOLEAN_TRUE
	};
	...
	/* Create MediaPlayer */
	r = (*engine_itf)->CreateMediaPlayer(engine_itf, &ctx->player, &source, NULL, &sink, NULL, NULL, NULL, 0, NULL, NULL);
	bail_if_error(r);
	...
	bail_if_error(r);
	r = (*(ctx->play_itf))->SetCallbackEventsMask(ctx->play_itf,
...
const aal_player_ops_t omxal_player_ops = {
	.create = omxal_player_create,
