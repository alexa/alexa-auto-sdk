...
static aal_module_t** g_aal_modules = NULL;

aal_module_t** aal_modules() {
    if (g_aal_modules == NULL) {
        int module_count = 0;
#ifdef CONFIG_GSTREAMER
        ++module_count;
#endif
        ...
        ++module_count;
#endif
        aal_module_t** modules = calloc(module_count + 1, sizeof(aal_module_t*));
        g_aal_modules = modules;

#ifdef CONFIG_GSTREAMER
        *modules++ = &gstreamer_module,
#endif
#ifdef CONFIG_OMXAL
        *modules++ = &omxal_module,
#endif
#ifdef CONFIG_QSA
        *modules++ = &qsa_module,
#endif
...
int aal_install_module(aal_module_t* module) {
    ...
    aal_module_t** new_modules = calloc(module_count + 1 + 1, sizeof(aal_module_t*));
    memcpy(new_modules, g_aal_modules, sizeof(aal_module_t*) * module_count);
    new_modules[module_count] = module;
    new_modules[module_count + 1] = NULL;
    free(g_aal_modules);
    g_aal_modules = new_modules;
