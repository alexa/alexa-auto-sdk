...
bool AASBAlerts::initialize(std::shared_ptr<aace::engine::messageBroker::MessageBrokerInterface> messageBroker) {
    try {
...
void AASBAlerts::alertStateChanged(const std::string& alertToken, AlertState state, const std::string& reason) {
    try {
        ...
        message.payload.state = static_cast<aasb::message::alexa::alerts::AlertState>(state);
