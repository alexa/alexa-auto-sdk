...
void AASBAlexaClient::connectionStatusChanged(ConnectionStatus status, ConnectionChangedReason reason) {
    try {
        ...
        message.payload.status = static_cast<aasb::message::alexa::alexaClient::ConnectionStatus>(status);
        message.payload.reason = static_cast<aasb::message::alexa::alexaClient::ConnectionChangedReason>(reason);
