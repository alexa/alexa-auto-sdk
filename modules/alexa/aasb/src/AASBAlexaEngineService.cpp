...
bool AASBAlexaEngineService::configureMessageInterface(
    ...
    try {
        ...
        ThrowIfNot(
            MessageHandlerEngineService::configureMessageInterface(name, enabled, configuration),
            "configureMessageInterfaceFailed");
...
bool AASBAlexaEngineService::postRegister() {
    try {
        ...
        //MediaPlaybackRequestor
        if (isInterfaceEnabled("MediaPlaybackRequestor")) {
            auto mediaPlaybackRequestor = AASBMediaPlaybackRequestor::create(aasbServiceInterface->getMessageBroker());
            ThrowIfNull(mediaPlaybackRequestor, "invalidMediaPlaybackRequestorHandler");
            context->registerPlatformInterface(mediaPlaybackRequestor);
        }
...
bool AASBAlexaEngineService::registerLocalMediaSourceMessageHandlers(
    ...
    try {
        messageBroker->subscribe(
            ...
            [this](const aace::engine::messageBroker::Message& message) {
                try {
