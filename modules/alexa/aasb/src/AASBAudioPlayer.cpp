...
bool AASBAudioPlayer::initialize(std::shared_ptr<aace::engine::messageBroker::MessageBrokerInterface> messageBroker) {
    try {
...
void AASBAudioPlayer::playerActivityChanged(PlayerActivity state) {
    try {
        ...
        message.payload.state = static_cast<aasb::message::alexa::audioPlayer::PlayerActivity>(state);
