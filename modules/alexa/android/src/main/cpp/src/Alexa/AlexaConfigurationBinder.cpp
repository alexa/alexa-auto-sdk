...
JNIEXPORT jlong JNICALL Java_com_amazon_aace_alexa_config_AlexaConfiguration_createDuckingConfigBinder(
    JNIEnv* env,
    jobject obj,
    jboolean enabled) {
    try {
        auto config = aace::alexa::config::AlexaConfiguration::createDuckingConfig(enabled);
        ThrowIfNull(config, "createDuckingConfigFailed");
