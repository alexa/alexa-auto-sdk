...
bool AASBAuthorization::initialize(std::shared_ptr<aace::engine::messageBroker::MessageBrokerInterface> messageBroker) {
    try {
...
void AASBAuthorization::authorizationStateChanged(const std::string& service, AuthorizationState state) {
    try {
        ...
        message.payload.service = service;
        message.payload.state = static_cast<aasb::message::authorization::authorization::AuthorizationState>(state);
...
void AASBAuthorization::setCachedRefreshToken(std::string& data) {
    m_cachedRefreshTokenData = data;
}
