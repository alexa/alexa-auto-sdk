...
void AudioOutputHandler::mayDuck() {
    try_with_context {
        ThrowIfNot(m_obj.invoke<void>("mayDuck", "()V", nullptr), "invokeMethodFailed");
    }
    catch_with_ex {
        AACE_JNI_ERROR(TAG, "mayDuck", ex.what());
    }
}
...
bool AudioOutputHandler::startDucking() {
    try_with_context {
        jboolean result;
        ThrowIfNot(m_obj.invoke("startDucking", "()Z", &result), "invokeMethodFailed");
        return result;
    }
    catch_with_ex {
        AACE_JNI_ERROR(TAG, "startDucking", ex.what());
        return false;
    }
}

bool AudioOutputHandler::stopDucking() {
    try_with_context {
        jboolean result;
        ThrowIfNot(m_obj.invoke("stopDucking", "()Z", &result), "invokeMethodFailed");
        return result;
    }
    catch_with_ex {
        AACE_JNI_ERROR(TAG, "stopDucking", ex.what());
        return false;
    }
}
...
JNIEXPORT void JNICALL
Java_com_amazon_aace_audio_AudioOutput_audioFocusEvent(JNIEnv* env, jobject /* this */, jlong ref, jobject action) {
    try {
        auto audioOutputBinder = AUDIO_OUTPUT_BINDER(ref);
        ThrowIfNull(audioOutputBinder, "invalidAudioOutputBinder");

        aace::audio::AudioOutput::FocusAction checkedAction;
        ThrowIfNot(aace::jni::audio::JFocusAction::checkType(action, &checkedAction), "invalidFocusAction");
        audioOutputBinder->getAudioOutputHandler()->audioFocusEvent(checkedAction);
    } catch (const std::exception& ex) {
        AACE_JNI_ERROR(TAG, "Java_com_amazon_aace_audio_AudioOutput_audioFocusEvent", ex.what());
    }
}
}
