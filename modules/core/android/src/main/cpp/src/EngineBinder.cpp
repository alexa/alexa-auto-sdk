...
Java_com_amazon_aace_core_Engine_getMessageBroker(JNIEnv* env, jobject /* this */, jlong ref) {
    try {
        auto engineBinder = ENGINE_BINDER(ref);
        ThrowIfNull(engineBinder, "invalidEngineBinder");

        auto engine = engineBinder->getEngine();
        ThrowIfNull(engine, "invalidEngine");

        JObject javaMessageBroker("com/amazon/aace/core/MessageBroker");
        ThrowIfJavaEx(env, "createMessageBrokerFailed");

        auto messageBroker = engine->getMessageBroker();
        ThrowIfNull(messageBroker, "getMessageBrokerFailed");

        // create the message broker binder
        long messageBrokerBinder = reinterpret_cast<long>(new aace::jni::core::MessageBrokerBinder(messageBroker));

        // set the java message broker object native ref to the message broker binder
        javaMessageBroker.invoke<void>("setNativeRef", "(J)V", nullptr, messageBrokerBinder);

        return env->NewLocalRef(javaMessageBroker.get());
    } catch (const std::exception& ex) {
        AACE_JNI_ERROR(TAG, "Java_com_amazon_aace_core_Engine_getMessageBroker", ex.what());
        return nullptr;
    }
}
}
