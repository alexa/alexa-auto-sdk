...
public:
    ...
    inline LogEntry& d(const char* key, ValueType&& value);

    template <typename ValueType>
    inline LogEntry& sensitive(const char* key, ValueType&& value);
    ...
    LogEntry& abortAfterEmission();
    bool shouldAbortAfterEmission() const;
...
private:
    ...
    bool m_hasMetadata;
    // Flag indicating whether the logger should abort the process after emitting this LogEntry.
    bool m_abortAfterEmission;
...
template <typename ValueType>
LogEntry& LogEntry::d(const char* key, ValueType&& value) {
    prefixKeyValuePair();
    m_stream << key << KEY_VALUE_SEPARATOR << std::forward<ValueType>(value);
    return *this;
}

#ifdef AAC_EMIT_SENSITIVE_LOGS
template <typename ValueType>
LogEntry& LogEntry::sensitive(const char* key, ValueType&& value) {
    return d(key, std::forward<ValueType>(value));
}
#else
template <typename ValueType>
LogEntry& LogEntry::sensitive(const char* key, ValueType&& value) {
