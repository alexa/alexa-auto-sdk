...
bool AudioOutputEngineImpl::prepare(
    const std::string& url,
    bool repeating,
    const AudioOutputChannelInterface::PlaybackContext& playbackContext) {
    try {
        aace::audio::AudioOutput::PlaybackContext audioOutputPlaybackContext;
        audioOutputPlaybackContext.keyConfig = playbackContext.keyConfig;
        audioOutputPlaybackContext.manifestConfig = playbackContext.manifestConfig;
        audioOutputPlaybackContext.audioSegmentConfig = playbackContext.audioSegmentConfig;
        audioOutputPlaybackContext.allConfig = playbackContext.allConfig;

        return m_platformAudioOutput->prepare(url, repeating, audioOutputPlaybackContext);
    } catch (std::exception& ex) {
        AACE_ERROR(LX(TAG).d("reason", ex.what()));
        return false;
    }
