...
std::shared_ptr<aace::core::config::EngineConfiguration> createSinkConfig( const std::string& id, const std::string& type, aace::logger::config::LoggerConfiguration::Level level, rapidjson::Value config = {} )
{
    ...
    ruleNode.AddMember( "level", rapidjson::Value().SetString( levelStr.c_str(), levelStr.length() ), document.GetAllocator() );
...
std::shared_ptr<aace::core::config::EngineConfiguration> LoggerConfiguration::createConsoleSinkConfig( const std::string& id, Level level ) {
    return createSinkConfig( id, "aace.logger.sink.console", level );
}

std::shared_ptr<aace::core::config::EngineConfiguration> LoggerConfiguration::createSyslogSinkConfig( const std::string& id, Level level ) {
    return createSinkConfig( id, "aace.logger.sink.syslog", level );
}

std::shared_ptr<aace::core::config::EngineConfiguration> LoggerConfiguration::createFileSinkConfig( const std::string& id, Level level, const std::string& path, const std::string& prefix, uint32_t maxSize, uint32_t maxFiles, bool append )
{
    rapidjson::Document document;
    ...
    configNode.AddMember( "prefix", rapidjson::Value().SetString( prefix.c_str(), prefix.length() ), document.GetAllocator() );
...
std::shared_ptr<aace::core::config::EngineConfiguration> LoggerConfiguration::createLoggerRuleConfig( const std::string& sink, Level level, const std::string& sourceFilter, const std::string& tagFilter, const std::string& messageFilter )
{
    rapidjson::Document document;
    ...
    defaultSinkRuleNode.AddMember( "sink", rapidjson::Value().SetString( sink.c_str(), sink.length() ), document.GetAllocator() );
    ...
    if( sourceFilter.empty() == false ) {
        ruleNode.AddMember( "source", rapidjson::Value().SetString( sourceFilter.c_str(), sourceFilter.length() ), document.GetAllocator() );
    }
    
    if( tagFilter.empty() == false ) {
        ruleNode.AddMember( "tag", rapidjson::Value().SetString( tagFilter.c_str(), tagFilter.length() ), document.GetAllocator() );
    }
    
    if( messageFilter.empty() == false ) {
        ruleNode.AddMember( "message", rapidjson::Value().SetString( messageFilter.c_str(), messageFilter.length() ), document.GetAllocator() );
    }
