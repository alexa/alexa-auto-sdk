...
std::shared_ptr<aace::engine::logger::sink::Sink> LoggerEngineService::createSink( const rapidjson::Value& config )
{
    try
    {
        ...
        else if( type == "aace.logger.sink.file" )
        {
            ...
            std::string prefix = config.HasMember( "prefix" ) && config["prefix"].IsString() ? config["prefix"].GetString() : "aace";
            uint32_t maxSize = config.HasMember( "maxSize" ) && config["maxSize"].IsUint() ? config["maxSize"].GetUint() : 1048576;
            uint32_t maxFiles = config.HasMember( "maxFiles" ) && config["maxFiles"].IsUint() ? config["maxFiles"].GetUint() : 3;
...
std::shared_ptr<aace::engine::logger::sink::Rule> LoggerEngineService::createRule( const rapidjson::Value& config )
{
    try
    {
        ...
        std::string level = obj.HasMember( "level" ) && obj["level"].IsString() ? obj["level"].GetString() : aace::engine::logger::sink::Rule::EMPTY;
        std::string source = obj.HasMember( "source" ) && obj["source"].IsString() ? obj["source"].GetString() : aace::engine::logger::sink::Rule::EMPTY;
        std::string tag = obj.HasMember( "tag" ) && obj["tag"].IsString() ? obj["tag"].GetString() : aace::engine::logger::sink::Rule::EMPTY;
        std::string message = obj.HasMember( "message" ) && obj["message"].IsString() ? obj["message"].GetString() : aace::engine::logger::sink::Rule::EMPTY;
