...
bool SQLiteStorage::initialize()
{
    try
    {
        ...
        if( is.good() ) {
            ThrowIf( sqlite3_open_v2( m_path.c_str(), &m_db, SQLITE_OPEN_READWRITE, nullptr ) != SQLITE_OK, "openDatabaseFailed" );
        }
        else {
            ThrowIf( sqlite3_open_v2( m_path.c_str(), &m_db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, nullptr ) != SQLITE_OK, "createDatabaseFailed" );
        }
...
bool SQLiteStorage::checkTable( const std::string &table, bool create )
{
    try
    {
        ...
        std::string sql = createStatement( "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='%s';", table.c_str() );
        
        bool exists = false;
        
        query( sql, []( void *data, int argc, char **argv, char **azColName ) {
            *((bool *) data) = argc == 1 && std::atoi( *argv ) == 1;
            return SQLITE_OK;
        }, &exists );

        if( exists == false && create ) {
            sql = createStatement( "CREATE TABLE '%s' (key STRING PRIMARY KEY NOT NULL,value STRING NOT NULL);", table.c_str() );
...
bool SQLiteStorage::put( const std::string& table, const std::string& key, const std::string& value )
{
    try
    {
        ...
        if( containsKey( table, key ) ) {
            sql = createStatement( "UPDATE '%s' SET value='%s' WHERE key='%s';", table.c_str(), value.c_str(), key.c_str() );
        }
        else {
            sql = createStatement( "INSERT INTO '%s' (key,value) VALUES ('%s','%s');", table.c_str(), key.c_str(), value.c_str() );
        }
