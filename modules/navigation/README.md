...
Use the `createNavigationConfig` function to configure the navigation provider programmatically as follows:

```cpp
auto navigationConfig = aace::navigation::config::NavigationConfiguration::createNavigationConfig("HERE");
```

For more information about the function, see [`aace.navigation.config.NavigationConfiguration.createNavigationConfig`](./platform/include/AACE/Navigation/NavigationConfiguration.h).
...
### Starting Navigation
To start navigation, the Engine calls the `startNavigation()` function, which passes a JSON payload with the following schema:
...
For `transportationMode` (optional), the value can be `"BIKING"`, `"DRIVING"`, `"TRANSIT"`, or `"WALKING"`. The properties in `waypoints` use the same schema as used for state reporting. See the table about the JSON string in the `getNavigationState()` function in [this section](#understanding-the-navigation-state-payload). 
...
### Stopping Navigation
To stop navigation, the Engine calls the `cancelNavigation()` function. 
...
### Adding a Waypoint
If the user wants to add a waypoint, the Engine calls the `startNavigation()` function. 
...
### Canceling a Waypoint
If the user wants to cancel a waypoint, the Engine calls the `startNavigation()` function after receiving the directive from Alexa with the updated waypoints. Your implementation should start navigation using the updated waypoints.
...
### Showing Previous Waypoints
If the user wants to display previous waypoints, the Engine calls the `showPreviousWaypoints()` function. Each waypoint displayed includes at least the address. 
...
### Navigating to a Previous Waypoint
If the user wants to  navigate to a previous waypoint, the Engine calls the `navigateToPreviousWaypoint()` function.
...
If the user wants to get turn and lane guidance, the Engine calls the `announceManeuver()` function, which passes a payload with the following schema:
...
### Getting Road Regulation Information
If the user wants to get road regulation information, the Engine calls the `announceRoadRegulation()` function, which passes a payload with the following schema:
...
### Controlling the Display
If the user wants to control the map display on the screen, the Engine calls the `controlDisplay()` function.
...
When the user requests navigation information based on the current route, the Engine invokes the `getNavigation()` function. The implementation should respond with the navigation state as a return value of the `getNavigation()`, passing a JSON string payload.
