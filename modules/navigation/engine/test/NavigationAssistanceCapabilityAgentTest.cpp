...
public:
    ...
    const std::string generateAnnounceRoadRegulationPayload(
...
TEST_F(NavigationAssistanceCapabilityAgent, testAnnounceManeuverDirective) {
    auto attachmentManager = std::make_shared<testing::StrictMock<alexaClientSDK::avsCommon::avs::attachment::test::MockAttachmentManager>>();
    auto avsMessageHeader = std::make_shared<alexaClientSDK::avsCommon::avs::AVSMessageHeader>( ANNOUNCE_MANEUVER.nameSpace, ANNOUNCE_MANEUVER.name, MESSAGE_ID );
    std::string MockPayload = generateAnnounceManeuverPayload(0.0, 0.0, "fake single line address", "fake multi-line adress", "fake name","TURN");
    std::shared_ptr<alexaClientSDK::avsCommon::avs::AVSDirective> directive =
...
TEST_F(NavigationAssistanceCapabilityAgent, testAnnounceRoadRegulationDirective) {
    auto attachmentManager = std::make_shared<testing::StrictMock<alexaClientSDK::avsCommon::avs::attachment::test::MockAttachmentManager>>();
    auto avsMessageHeader = std::make_shared<alexaClientSDK::avsCommon::avs::AVSMessageHeader>( ANNOUNCE_ROAD_REGULATION.nameSpace, ANNOUNCE_ROAD_REGULATION.name, MESSAGE_ID );
    std::string MockPayload = generateAnnounceRoadRegulationPayload("SPEED_LIMIT");
    std::shared_ptr<alexaClientSDK::avsCommon::avs::AVSDirective> directive =
    alexaClientSDK::avsCommon::avs::AVSDirective::create( "", avsMessageHeader, MockPayload, attachmentManager, "" );
    
    EXPECT_CALL(*m_testNavigationHandler, announceRoadRegulation( aace::navigation::Navigation::RoadRegulation::SPEED_LIMIT )).Times(testing::Exactly(1));
