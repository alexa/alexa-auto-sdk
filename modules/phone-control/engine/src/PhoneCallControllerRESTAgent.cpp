 ...
 */
static std::vector<std::string> buildCommonHTTPHeader( const std::string& deviceSerialNumber, const std::string& authToken ) {
    std::vector<std::string> httpHeaderData;
    std::string requestUUID = alexaClientSDK::avsCommon::utils::uuidGeneration::generateUUID();
    httpHeaderData = {
        AUTHORIZATION_HTTP_HEADER + COLON_SEPARATOR + SPACE_SEPARATOR + AUTHORIZATION_HTTP_HEADER_POSTFIX + SPACE_SEPARATOR + authToken,
 ...
 */
static alexaClientSDK::avsCommon::utils::libcurlUtils::HTTPResponse doPost( const std::string& url, const std::vector<std::string> headerLines, const std::string& data, std::chrono::seconds timeout ) {
    try {
 ...
 * @return On success returns @c HTTPResponse received from server, otherwise empty object of @c HTTPResponse.
 */
static alexaClientSDK::avsCommon::utils::libcurlUtils::HTTPResponse doGet( const std::string& url, const std::vector<std::string>& headers ) {
    try {
 ...
 */
static std::string convertHTTPErrorStringForLogging( const alexaClientSDK::avsCommon::utils::libcurlUtils::HTTPResponse& response ) {
...
AlexaAccountInfo getAlexaAccountInfo( std::shared_ptr<alexaClientSDK::avsCommon::sdkInterfaces::AuthDelegateInterface> authDelegate,
    ...
    try { 
        auto httpResponse = doGet( 
            ACMS_ENDPOINT + FORWARD_SLASH + ACCOUNTS_PATH, 
            httpHeaderData );

        ThrowIfNot( parseCommonHTTPResponse( httpResponse ), "parseCommonHTTPResponseFailed" + convertHTTPErrorStringForLogging( httpResponse ) );
...
bool doAccountAutoProvision( const AlexaAccountInfo& alexaAccountInfo, 
    ...
    try {
        auto httpResponse = doPost(
            ACMS_ENDPOINT + FORWARD_SLASH + ACCOUNTS_PATH + FORWARD_SLASH + alexaAccountInfo.directedId + FORWARD_SLASH + USERS_PATH,
            httpHeaderData, autoProvisionJson, DEFAULT_HTTP_TIMEOUT );
    
        ThrowIfNot( parseCommonHTTPResponse( httpResponse ), "httpDoPostFailed" + convertHTTPErrorStringForLogging( httpResponse ) );
        return true;
    } catch( std::exception& ex ) {
