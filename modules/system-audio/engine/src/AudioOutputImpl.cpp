...
void AudioOutputImpl::executeOnStop(aal_status_t reason) {
    ...
    if (!checkState({
            ...
            State::Pausing,   // pause
            State::Starting,  // stopped before play is started
            State::Started,   // completed
            State::Resuming,  // stopped before play is resumed
            State::Faulted,
        })) {
        if (reason == AAL_ERROR) {
    ...
    try {
        ...
        } else if (checkState({State::Starting, State::Resuming, State::Started})) {
            ...
            m_executor
                .submit([this]() {
                    ...
                    if (!m_mediaQueue.empty()) {
                        ...
                        preparePlayer([this](aal_attributes_t* attr, aal_audio_parameters_t*) {
                            attr->uri = m_mediaQueue.front().c_str();
                            return nullptr;
                        });
                        if (m_player) {
...
bool AudioOutputImpl::executePrepare(
    const std::string& url,
    const std::shared_ptr<aace::audio::AudioStream>& stream,
    bool repeating) {
    AACE_INFO(LXT.d("repeating", repeating));
    if (!checkState({
    ...
    try {
        ...
        if (!url.empty()) {
            ThrowIfNot(prepareUrl(url), "prepareUrlFailed");
        } else if (stream) {
            AACE_VERBOSE(LXT.d("encoding", stream->getEncoding()));
            switch (stream->getEncoding()) {
                ...
                case audio::AudioStream::Encoding::MP3: {
                    ...
                    m_tmpFile = tmpFile;
                    ThrowIfNot(prepareUrl("file://" + m_tmpFile), "prepareUrlMP3Failed");
                    break;
                }
                case audio::AudioStream::Encoding::LPCM:
                    m_currentStream = stream;
                    ThrowIfNot(prepareStream(stream), "prepareStreamLPCMFailed");
                    break;
                default:
                    Throw("unsupportedStreamEncoding");
            }
        } else {
            Throw("invalidUrlAndStream");
        }
...
void AudioOutputImpl::mayDuck() {
    AACE_INFO(LXT.d("mayDuck", "true"));
}
...
std::vector<std::string> AudioOutputImpl::parsePlaylistUrl(const std::string& url) {
    ...
    if (parser->parsePlaylist(url, observer, {PlaylistParser::PlaylistParserInterface::PlaylistType::EXT_M3U}) ==
        PlaylistParser::START_FAILURE) {
...
bool AudioOutputImpl::prepareUrl(const std::string& url) {
    try {
        ...
        m_mediaUrl = url;  // save it for repeating
    } catch (std::exception& ex) {
        AACE_ERROR(LXT.d("reason", ex.what()));
        return false;
    }
...
bool AudioOutputImpl::prepareStream(const std::shared_ptr<aace::audio::AudioStream>& stream) {
    try {
        auto af = stream->getAudioFormat();
        preparePlayer([this, &af](aal_attributes_t* attr, aal_audio_parameters_t* params) {
...
void AudioOutputImpl::preparePlayer(
    ...
    auto* params = configure(&attr, &audio_params);
    m_player = aal_player_create(&attr, params);
...
bool AudioOutputImpl::executePlay() {
    try {
        AACE_VERBOSE(LXT);
        if (checkState({State::Starting, State::Resuming, State::Started})) {
...
bool AudioOutputImpl::executeStop() {
    try {
        ...
        if (checkState({State::Initialized, State::Prepared, State::Stopped})) {
            ...
            return false;
        } else if (!checkState({State::Starting, State::Resuming, State::Started, State::Paused})) {
            AACE_ERROR(LXT.d("reason", "invalid state").d("state", m_state));
            return false;
        }
...
bool AudioOutputImpl::startDucking() {
    m_executor.submit([this] { return executeStartDucking(); });
    return true;
}

bool AudioOutputImpl::executeStartDucking() {
    try {
        m_volumeCoefficient = 0.2;
        if (m_currentMutedState == MutedState::MUTED) {
            return true;
        }
        auto volume = m_volumeCoefficient * m_currentVolume;
...
bool AudioOutputImpl::stopDucking() {
    m_executor.submit([this] { return executeStopDucking(); });
    return true;
}

bool AudioOutputImpl::executeStopDucking() {
    try {
        m_volumeCoefficient = 1.0;
        if (m_currentMutedState == MutedState::MUTED) {
            return true;
        }
        auto volume = m_volumeCoefficient * m_currentVolume;
...
bool AudioOutputImpl::executeVolumeChanged(float volume) {
    try {
        ...
        // Set volume only when AAL handle is available, ignore when NULL
        if (m_player) {
            aal_player_set_volume(m_player, m_volumeCoefficient * volume);
        }
        // We save the value for later use
