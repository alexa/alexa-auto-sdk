...
public class AACSTelephonyService extends Service {
    ...
    private boolean isExpectingPhoneAccount;
    ...
    private void handleIntent(Intent intent) {
        // Non-AASB directive intents

        if (TelephonyConstants.ACTION_REPLY_PHONE_ACCOUNT_HANDLER.equals(intent.getAction())
                && isExpectingPhoneAccount) {
            if (intent.getBooleanExtra(TelephonyConstants.ISREDIAL, false)) {
                Log.i(TAG, "about to redial");
                redial(intent);
            } else {
                Log.i(TAG, "about to dial");
                dial(intent);
            }
            Log.i(TAG, "is redial? " + intent.getBooleanExtra(TelephonyConstants.ISREDIAL, false));
            isExpectingPhoneAccount = false;
            return;
        }
    ...
    public void dial(Intent intent) {
        Bundle bundle = intent.getBundleExtra(TelephonyConstants.PHONE_ACCOUNT_BUNDLE);
        PhoneAccountHandle handle = bundle.getParcelable(TelephonyConstants.PHONE_ACCOUNT_HANDLE);
        String callId = intent.getStringExtra(TelephonyConstants.CALLID);
        String callNumber = intent.getStringExtra(TelephonyConstants.CALLERID);
    ...
    public void queryPhoneAccount(String payload, Boolean isRedial) {
        try {
            ...
            Intent intent = new Intent(TelephonyConstants.ACTION_QUERY_PHONE_ACCOUNT_HANDLER);
            intent.addCategory(TelephonyConstants.CATEGORY_AACS_TELEPHONY);

            List<TargetComponent> targets =
                    AACSComponentRegistryUtil.queryPackageManager(this, intent, Constants.AACS_TELEPHONY_PERMISSION);
            if (targets == null) {
                Log.e(TAG, "No app is listening to the query phone account intent");
                return;
            }
            intent.putExtra(TelephonyConstants.CALLERID, callNumber);
            intent.putExtra(TelephonyConstants.CALLID, callId);
            intent.putExtra(TelephonyConstants.ISREDIAL, isRedial);

            Util.sendNonAASBIntent(this, targets, intent);
            Log.v(TAG, "Query phone account intent sent. ");

            isExpectingPhoneAccount = true;
        } catch (JSONException e) {
            Log.e(TAG, "Error parsing Dial directive payload: " + e.getMessage());
        }
    }

    public void redial(Intent intent) {
        Bundle bundle = intent.getBundleExtra(TelephonyConstants.PHONE_ACCOUNT_BUNDLE);
        PhoneAccountHandle handle = bundle.getParcelable(TelephonyConstants.PHONE_ACCOUNT_HANDLE);
        String callId = intent.getStringExtra(TelephonyConstants.CALLID);
