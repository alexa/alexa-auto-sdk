...
public class APLPresenter implements IPresenter, ISendEventCallback, IDataSourceFetchCallback, IDataSourceErrorCallback,
                                     IVisualContextListener, IAPLTokenProvider {
    private static final String TAG = "APLPresenter";
    ...
    public static void initialize(Context context) {
        mContext = context;
        APLController.initializeAPL(context, RuntimeConfig.builder().accessibilityHandledByRuntime(true).build());
    }
    /**
        ...
        getAplLayout().post(() -> {
            try {
                ...
                if (!commandsString.isEmpty()) {
                    mAction = mAplController.executeCommands(commandsString);
                    if (mAction != null) {
                        mAction.then(() -> {
                            Log.i(TAG,
    ...
    private void doRender(Content content) {
        final RootConfig config = mActivityComponent.getRootConfig();
        ...
        try {
            aplLayout.post(() -> {
                ...
                content.createDocumentBackground(presenter.createViewportMetrics(), config);
                aplLayout.getPresenter().loadBackground(content.getDocumentBackground());
                aplLayout.getPresenter().addDocumentLifecycleListener(this);
                try {
                    mAplController = APLController.renderDocument(content, options, config, presenter);
                    sendDeviceWindowState();
                } catch (Exception e) {
    ...
    private void applyScaling() {
        List<ViewportMode> fallbackModes = Collections.singletonList(ViewportMode.kViewportModeHub);
