...
abstract public class ContactUploader extends PlatformInterface {
     ...
     *
     * After returning @c true, the Engine will remove the previously uploaded contacts, if any, and the platform
     * implementation may start calling @c addContact to upload the contacts.
     *
     ...
     * @li addresses.type (required): Supported address type is "phonenumber" only. And the max supported phone number
     * per contact is 30.
     ...
     *
     * @return @c true if successfully queued, @c false when contact format is not valid, upload not started or for any
     *         internal error.
     */
    ...
    /**
     * Notifies the Engine to remove the uploaded local contacts from the Alexa Cloud. This API to be called when the
     * local contacts become unavailable (Example use case, Phone is disconnected from the Head Unit) and contacts were
     * already successfully uploaded to Alexa Cloud.
     *
