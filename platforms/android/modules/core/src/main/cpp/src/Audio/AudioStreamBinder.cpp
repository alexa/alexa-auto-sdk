...
extern "C"
{
    ...
    Java_com_amazon_aace_audio_AudioStream_getEncoding( JNIEnv* env, jobject /* this */, jlong ref )
    {
        try
        {
            ...
            jobject encodingObj;
            ThrowIfNot( aace::jni::audio::JAudioStreamEncoding::checkType( audioStreamBinder->getAudioStream()->getEncoding(), &encodingObj ), "invalidEncoding" );
    ...
    Java_com_amazon_aace_audio_AudioStream_getAudioFormat( JNIEnv* env, jobject /* this */, jlong ref )
    {
        try
        {
            ...
            // get the encoding
            jobject encodingObj;
            ThrowIfNot( aace::jni::audio::JAudioFormatEncoding::checkType( audioFormat.getEncoding(), &encodingObj ), "invalidAudioFormatEncoding" );

            // get the format
            jobject sampleFormatObj;
            ThrowIfNot( aace::jni::audio::JAudioFormatSampleFormat::checkType( audioFormat.getSampleFormat(), &sampleFormatObj ), "invalidAudioFormatSampleFormat" );

            // get the layout
            jobject layoutObj;
            ThrowIfNot( aace::jni::audio::JAudioFormatLayout::checkType( audioFormat.getLayout(), &layoutObj ), "invalidAudioFormatLayout" );

            // get the endianness
            jobject endiannessObj;
            ThrowIfNot( aace::jni::audio::JAudioFormatEndianness::checkType( audioFormat.getEndianness(), &endiannessObj ), "invalidAudioFormatEndianness" );

            jobject audioFormatObj = audioFormatClass->newInstance( 
                "(Lcom/amazon/aace/audio/AudioFormat$Encoding;Lcom/amazon/aace/audio/AudioFormat$SampleFormat;Lcom/amazon/aace/audio/AudioFormat$Layout;Lcom/amazon/aace/audio/AudioFormat$Endianness;III)V", encodingObj, sampleFormatObj, layoutObj, endiannessObj, audioFormat.getSampleRate(), audioFormat.getSampleSize(), audioFormat.getNumChannels() );
    ...
    Java_com_amazon_aace_audio_AudioStream_getProperties( JNIEnv* env, jobject /* this */, jlong ref )
    {
        try
        {
            ...
            auto audioStreamPropertyClass = aace::jni::native::JavaClass::find( "com/amazon/aace/audio/AudioStream$AudioStreamProperty" );
            ...
            for( int j = 0; j < properties.size(); j++ )
            {
                jobject audioStreamPropertyObj = audioStreamPropertyClass->newInstance( 
                    "(Ljava/lang/String;Ljava/lang/String;)V", JString(properties[j].getKey()).get(), JString(properties[j].getValue()).get() );
