...
namespace metrics {
    ...
    MetricsUploaderHandler::MetricsUploaderHandler( jobject obj ) : m_obj( obj, "com/amazon/metricuploadservice/MetricsUploader" ) {
    }

    bool MetricsUploaderHandler::record( const std::vector<aace::metrics::MetricsUploader::Datapoint>& datapoints, const std::unordered_map<std::string, std::string>& metadata )
    {
        try_with_context
        {
            auto dataPointClass = aace::jni::native::JavaClass::find( "com/amazon/metricuploadservice/MetricsUploader$Datapoint" );
            ...
            for( int j = 0; j < datapoints.size(); j++ )
            {
                ...
                ThrowIfNot( JDatapointType::checkType( datapoints[j].getType(), &checkedDataPointTypeObj ), "invalidDataPointType" );

                jobject dataPointObj = dataPointClass->newInstance( "(Lcom/amazon/metricuploadservice/MetricsUploader$Datapoint;I)V", 
                    checkedDataPointTypeObj, JString(datapoints[j].getName()).get(), JString(datapoints[j].getValue()).get(), datapoints[j].getCount() );
            ...
            for( auto itr : metadata ) {
                ThrowIfNot( map.invoke<jobject>( "put", 
                    "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;", nullptr, JString(itr.first).get(), JString(itr.second).get() ), "invokeMethodFailed" );
            }

            jboolean result;
            ThrowIfNot( m_obj.invoke( "record", "([Lcom/amazon/metricuploadservice/MetricsUploader$Datapoint;Ljava/util/HashMap;)Z", &result, arr.get(), map.get() ), "invokeMethodFailed" );
            return result;
        }
        catch_with_ex {
