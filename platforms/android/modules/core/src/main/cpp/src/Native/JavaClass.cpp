...
namespace native {
    ...
    std::shared_ptr<JavaClass> JavaClass::find( const char* name )
    {
        try_with_context
        {
            ...
            jclass cls = (jclass) env->CallObjectMethod( s_classLoaderObjectRef.get(), s_findClassMethodID, JavaString(name).get() );
            ThrowIfJavaEx( env, "CallObjectMethodFailed" );
    ...
    template <class T> bool JavaClass::invokeV( JavaMethodPtr method, T* result, va_list ) {
        AACE_JNI_ERROR("TAG","invokeV","undefinedTypeSpecification");
        return false;
    }

    template <class T> bool JavaClass::invoke( JavaMethodPtr method, T* result, ... )
    {
        try_with_context
        {
            // start - handle vargs
