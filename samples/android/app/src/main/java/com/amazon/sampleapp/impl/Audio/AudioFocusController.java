 ...
 */
public class AudioFocusController implements AudioManager.OnAudioFocusChangeListener, AutoCloseable {
    @VisibleForTesting
    static final float VOLUME_MULTIPLIER_DUCK = 0.2f;
    @VisibleForTesting
    static final float VOLUME_MULTIPLIER_NORMAL = 1.0f;
    private static final String TAG = AudioFocusController.class.getSimpleName();
    ...
    /**
     * Construct an instance of {@link AudioFocusController}.
     *
     * @param audioManager {@link AudioManager} Android Audio Manager from which
     *                     audio focus would be requested.
     */
    public AudioFocusController(
            @NonNull AudioManager audioManager, @NonNull PlaybackController playbackController, String name) {
    ...
    public void onAudioFocusChange(int focusChange) {
        Log.d(TAG,
                "Focus Changed. Current audio focus state: " + focusStateToString(mCurrentState)
                        + " Focus change: " + focusChangeToString(focusChange) + " (" + mName + ")");
        ...
        switch (focusChange) {
            case AudioManager.AUDIOFOCUS_GAIN:
                switch (mCurrentState) {
                    ...
                    case AUDIO_FOCUS_STATE_NONE:
                        break; // Regain focus if we previously lost it
                }
    ...
    public void relinquishAudioFocusIfCurrentlyAcquired() {
        Log.d(TAG,
                "Relinquishing Audio Focus. Current state: " + focusStateToString(mCurrentState) + " (" + mName + ")");
