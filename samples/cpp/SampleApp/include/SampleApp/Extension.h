...
public:
    ...
    virtual bool initialize(
        ...
        std::weak_ptr<aace::propertyManager::PropertyManager> propertyManagerHandler) = 0;
    // Validate extension
    virtual bool validate(const std::vector<nlohmann::json>& configs) = 0;
    // Get the platform interface handler, or return nullptr if no handler is needed
    virtual std::shared_ptr<aace::core::PlatformInterface> getPlatformInterfaceHandler() = 0;
    // Get extension name
    std::string getName() {
        return m_name;
    }
...
public:
    ...
    static bool registerPlatformInterfaces(std::shared_ptr<aace::core::Engine> engine);
    // Get extension
