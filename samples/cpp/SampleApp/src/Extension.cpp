...
bool Manager::initializeExtensions(
    ...
    for (auto& it : getInstance()->m_extensionMap) {
        ...
        if (!extension->initialize(activity, loggerHandler, propertyManagerHandler)) {
            initialized = false;
        }
    }
    return initialized;
...
bool Manager::registerPlatformInterfaces(std::shared_ptr<aace::core::Engine> engine) {
    bool registered = true;
    // Register platform interface handlers
    for (auto& it : getInstance()->m_extensionMap) {
        auto extension = it.second;
        auto platformHandler = extension->getPlatformInterfaceHandler();
        // Some extension only validate configuration but do not provide handler
        if (platformHandler != nullptr) {
            if (!engine->registerPlatformInterface(platformHandler)) {
                registered = false;
            }
        }
    }
