...
void MessagingHandler::subscribeToAASBMessages() {
    log(logger::LoggerHandler::Level::INFO, "Subscribe to SendMessage Messages");
    m_messageBroker->subscribe(
        [=](const std::string& message) { handleSendMessageMessage(message); },
        SendMessageMessage::topic(),
        SendMessageMessage::action());

    log(logger::LoggerHandler::Level::INFO, "Subscribe to UpdateMessagesStatus Messages");
    m_messageBroker->subscribe(
    ...
    log(logger::LoggerHandler::Level::INFO, "Subscribe to UploadConversations Messages");
    m_messageBroker->subscribe(
...
void MessagingHandler::conversationsReport(const std::string& token, const std::string& conversations) {
    // Publish the "ConversationsReport" message
...
void MessagingHandler::sendMessageFailed(const std::string& token, ErrorCode code, const std::string& message) {
    // Publish the "SendMessageFailed" message
...
void MessagingHandler::sendMessageSucceeded(const std::string& token) {
    // Publish the "SendMessageSucceeded" message
    SendMessageSucceededMessage msg;
    msg.payload.token = token;
    m_messageBroker->publish(msg.toString());
}

void MessagingHandler::updateMessagesStatusFailed(
    const std::string& token,
    ErrorCode code,
    const std::string& message) {
    // Publish the "UpdateMessagesStatusFailed" message
...
void MessagingHandler::updateMessagesStatusSucceeded(const std::string& token) {
    // Publish the "UpdateMessagesStatusSucceeded" message
    UpdateMessagesStatusSucceededMessage msg;
    msg.payload.token = token;
    m_messageBroker->publish(msg.toString());
}

void MessagingHandler::updateMessagingEndpointState(
    ConnectionState connectionState,
    PermissionState sendPermission,
    PermissionState readPermission) {
    // Publish the "UpdateMessagingEndpointState" message
